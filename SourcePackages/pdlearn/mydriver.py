...
class Mydriver:
    ...
    def __init__(self, noimg=True, nohead=True):
        ...
        try:
            ...
            # chromedriver_paths.append(os.getcwd().split()[:-2]+'\\')
            chrome_path_dondaha = ''
            for path_str_split in os.getcwd().split('\\')[:-1]:
                chrome_path_dondaha += (path_str_split + '\\')
            chrome_path_dondaha += '\\chrome\\chromedriver.exe'
            chromedriver_paths.append(chrome_path_dondaha)
            have_find = False
